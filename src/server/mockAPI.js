...
const mockAPI=(textValue,callback)=>{

    const url = `${baseURL}?key=${apiKey}&of=json&txt=${encodeURIComponent(textValue)}&lang=en`;
    const json=true;
    const method='GET';
    request({url,json,method},(error,{body})=>{
        ...
        else if(body.status.code===102)
        {
            ...
            console.log(scoreTagArray[position]);
            ...
            console.log(typeof sentimentData);
            callback(undefined,{sentimentData});
        }

        else {
            console.log(body.score_tag);
            console.log(body.agreement);
            console.log(body.subjective);
            console.log(body.ironic);
            const sentimentData = {
                scoreTag:'N/A',
                agreement:body.agreement===undefined?'N/A':body.agreement,
